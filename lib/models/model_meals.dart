...
import 'model_meal.dart';

class Meals with ChangeNotifier {
  List<Meal> dataFiltered;
  List<Meal> dataFavorites = [];
  List<Meal> data = [
    Meal(
      id: 'm1',
      categories: [
  ...
  bool isFavorite(String id) {
    return dataFavorites.any((Meal meal) => meal.id == id);
  }

  void toggleFavorite(String mealId) {
    // NonExisting meal gives back -1
    final existingIndex = dataFavorites.indexWhere((Meal meal) => meal.id == mealId);
    if (existingIndex >= 0) {
      dataFavorites.removeAt(existingIndex);
    } else {
      // Add first meal where the ID matches the ID
      dataFavorites.add(data.firstWhere((Meal meal) => meal.id == mealId));
    }
    notifyListeners();
  }

  // THIS IS THE SHIT!!!
  void filteredData(Map filter) {
    List<Meal> temp = data.where((Meal meal) {
